...
wildcard_constraints:
    ...
    ref_name="|".join(config["ref"].keys()),
    sample="|".join([item for cohort, info in config["samples_info"].items() for item in info]),
# caller_p="|".join(config[""])

bam_prefix = "{ref_name}.ILM.{aligner}"
    ...
    for aligner in config["aligners"]:
        ...
        for cohort, cohort_info in samples_info.items():
            for sample in cohort_info:
                hq_bam = config["dir_aligned_reads"] + "{cohort}/{sample}/{cohort}.{sample}.{prefix}.bam.bai".format(
                    cohort=cohort,
                    sample=sample,
                    prefix=this_prefix)
                targets.append(hq_bam)
                bam_report = config["dir_reports"] + "{cohort}.{prefix}.multiqc.html".format(prefix=this_prefix,
                    cohort=cohort)
